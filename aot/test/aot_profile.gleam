...
@external(erlang, "aot_profile_ffi", "trace_on")
...
@external(erlang, "aot_profile_ffi", "top_n")
fn top_n(n: Int) -> List(#(String, String, Int, Int, Int))

@external(erlang, "aot_profile_ffi", "module_total")
...
fn correctness_gate(label: String, path: String) -> Bool {
  let assert Ok(source) = simplifile.read(path)
  case aot_harness.run_compiled(source) {
    aot_harness.DiffRun(result: Ok(_), stdout: <<"ok\n":utf8>>) -> {
...
pub fn main() {
  io.println("aot call_time profile — traced=local, per-{M,F,A}")
  dump_am3_core()
}
