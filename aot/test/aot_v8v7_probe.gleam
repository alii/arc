...
@external(erlang, "aot_probe_ffi", "env_int")
fn env_int(name: String, default: Int) -> Int

@external(erlang, "aot_probe_ffi", "load_average")
...
fn bench_compiled(name: String, source: String) -> Outcome {
  ...
  case emit_r {
    Error(e) -> CompileFailed("aot", string.inspect(e))
    Ok(ir_module) -> {
...
fn bench_interp(source: String) -> Outcome {
  let #(warm_us, first) = time_us(fn() { aot_harness.run_interpreted(source) })
  case first {
    aot_harness.DiffRun(result: Error(e), stdout:) ->
      RunFailed(e, string.inspect(stdout))
    aot_harness.DiffRun(result: Ok(_), stdout:) ->
      case stdout {
        <<"ok\n":utf8>> -> {
          let reps = reps_for(warm_us)
          Measured(
            min_of_n(reps, fn() { aot_harness.run_interpreted(source) }),
            reps,
          )
        }
        _ -> RunFailed("stdout mismatch", string.inspect(stdout))
...
fn one(name: String) {
  ...
  case simplifile.read(path) {
    ...
    Ok(source) -> {
      ...
      io.println("  aot : " <> show(compiled))
      let interp = case aot_harness.env_is_truthy("PROBE_SKIP_INTERP") {
        True -> Measured(0, 0)
...
pub fn main() {
  io.println(
    "aot V8-v7 probe — one call = one full bench iteration, min-of-"
    <> int.to_string(batches())
    <> " batches (PROBE_N)",
  )
  io.println("  ROW bench\taot\tarc_interp\tqjs\tllint\tload_avg")
  one("richards")
  one("deltablue")
  one("crypto")
  one("raytrace")
