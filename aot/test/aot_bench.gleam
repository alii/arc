// run: cd aot && gleam run -m aot_bench
...
fn bench(
  ...
  let c = aot_harness.run_compiled(logged)
  let i = aot_harness.run_interpreted(logged)
...
pub fn main() {
  ...
  io.println(
    "bench      aot          arc-interp   m0-hand-IR   native-gleam   "
    <> "compiled/m0",
  )
