...
pub fn sum_n_diff_test() {
  let i = aot_harness.run_interpreted(sum_src)
  let c = aot_harness.run_compiled(sum_src)
  assert i.stdout == <<"55\n":utf8>>
  assert c.stdout == i.stdout
}

pub fn make_adder_diff_test() {
  let i = aot_harness.run_interpreted(make_adder_src)
  let c = aot_harness.run_compiled(make_adder_src)
  assert i.stdout == <<"7\n":utf8>>
  assert c.stdout == i.stdout
}
...
pub fn object_literal_diff_test() {
  let i = aot_harness.run_interpreted(object_literal_src)
  let c = aot_harness.run_compiled(object_literal_src)
  assert i.stdout == <<"5\n":utf8>>
  assert c.stdout == i.stdout
}

pub fn obj_prop_diff_test() {
  let i = aot_harness.run_interpreted(obj_prop_src)
  let c = aot_harness.run_compiled(obj_prop_src)
  assert i.stdout == <<"10\n":utf8>>
  assert c.stdout == i.stdout
}
...
pub fn method_call_proto_own_diff_test() {
  let i = aot_harness.run_interpreted(method_call_proto_own_src)
  let c = aot_harness.run_compiled(method_call_proto_own_src)
...
pub fn method_call_proto_chain_diff_test() {
  let i = aot_harness.run_interpreted(method_call_proto_chain_src)
  let c = aot_harness.run_compiled(method_call_proto_chain_src)
  assert i.stdout == <<"A\nB\n":utf8>>
  assert c.stdout == i.stdout
}
...
pub fn method_call_shaped_chain_diff_test() {
  let i = aot_harness.run_interpreted(method_call_shaped_chain_src)
  let c = aot_harness.run_compiled(method_call_shaped_chain_src)
  assert i.stdout
    == <<
...
pub fn method_call_miss_diff_test() {
  let i = aot_harness.run_interpreted(method_call_miss_src)
  let c = aot_harness.run_compiled(method_call_miss_src)
...
pub fn this_multi_field_diff_test() {
  let i = aot_harness.run_interpreted(this_multi_field_src)
  let c = aot_harness.run_compiled(this_multi_field_src)
...
pub fn ctor_add_setter_diff_test() {
  let i = aot_harness.run_interpreted(ctor_add_setter_src)
  let c = aot_harness.run_compiled(ctor_add_setter_src)
...
pub fn ctor_add_readonly_diff_test() {
  let i = aot_harness.run_interpreted(ctor_add_readonly_src)
  let c = aot_harness.run_compiled(ctor_add_readonly_src)
  assert i.stdout == <<"72y\n":utf8>>
  assert c.stdout == i.stdout
}
...
pub fn ctor_add_proxy_diff_test() {
  let i = aot_harness.run_interpreted(ctor_add_proxy_src)
  let c = aot_harness.run_compiled(ctor_add_proxy_src)
...
pub fn ctor_add_shaped_proto_diff_test() {
  let i = aot_harness.run_interpreted(ctor_add_shaped_proto_src)
  let c = aot_harness.run_compiled(ctor_add_shaped_proto_src)
  assert i.stdout == <<"21m\n":utf8>>
  assert c.stdout == i.stdout
}
...
pub fn set_read_coherence_diff_test() {
  let i = aot_harness.run_interpreted(set_read_coherence_src)
  let c = aot_harness.run_compiled(set_read_coherence_src)
  assert i.stdout == <<"454\n":utf8>>
  assert c.stdout == i.stdout
}
...
pub fn new_return_shape_diff_test() {
  let i = aot_harness.run_interpreted(new_return_shape_src)
  let c = aot_harness.run_compiled(new_return_shape_src)
  assert i.stdout == <<"2\n1\n":utf8>>
  assert c.stdout == i.stdout
}
...
pub fn new_nonctor_diff_test() {
  let i = aot_harness.run_interpreted(new_nonctor_src)
  let c = aot_harness.run_compiled(new_nonctor_src)
...
pub fn truth_value_results_are_booleans_diff_test() {
  let i = aot_harness.run_interpreted(truth_value_src)
  let c = aot_harness.run_compiled(truth_value_src)
  assert i.stdout
    == <<
...
pub fn instanceof_chain_diff_test() {
  let i = aot_harness.run_interpreted(instanceof_chain_src)
  let c = aot_harness.run_compiled(instanceof_chain_src)
  assert i.stdout == <<"yyn\n":utf8>>
  assert c.stdout == i.stdout
}
...
pub fn instanceof_bound_diff_test() {
  let i = aot_harness.run_interpreted(instanceof_bound_src)
  let c = aot_harness.run_compiled(instanceof_bound_src)
  assert i.stdout == <<"yn\n":utf8>>
  assert c.stdout == i.stdout
}
...
pub fn instanceof_has_instance_diff_test() {
  let i = aot_harness.run_interpreted(instanceof_has_instance_src)
  let c = aot_harness.run_compiled(instanceof_has_instance_src)
  assert i.stdout == <<"yn\n":utf8>>
  assert c.stdout == i.stdout
}
...
pub fn cond_eq_diff_test() {
  let i = aot_harness.run_interpreted(cond_eq_src)
  let c = aot_harness.run_compiled(cond_eq_src)
  assert i.stdout == <<"y n y y y\nne\n3\nonce\n":utf8>>
  assert c.stdout == i.stdout
}

fn diff(src: String, want: String) {
  let i = aot_harness.run_interpreted(src)
  let c = aot_harness.run_compiled(src)
...
pub fn unsupported_import_call_is_compile_error_test() {
  let c = aot_harness.run_compiled("var p=import('x');console.log('no')")
  assert c.stdout == <<>>
...
pub fn unhandled_rejection_report_diff_test() {
  ...
  let i = aot_harness.run_interpreted(src)
  let i_err = aot_harness.err_read()
  let c = aot_harness.run_compiled(src)
  assert i.stdout == <<"main\n":utf8>>
  assert c.stdout == i.stdout
  assert i_err == <<"Uncaught (in promise) nobody\n":utf8>>
  assert aot_harness.err_read() == i_err
  let c = aot_harness.run_compiled("Promise.reject(new RangeError('e'))")
