...
pub fn run_prints_and_drains_test() {
  ...
  let result =
    arc_aot.execute(Run("test/fixtures/hello.js"), aot_harness.test_hooks())
  assert result == Ok(Nil)
  assert aot_harness.buf_read() == <<"hello aot\nlater\n":utf8>>
}

pub fn run_uncaught_test() {
  let assert Error(err) =
    arc_aot.execute(Run("test/fixtures/throws.js"), aot_harness.test_hooks())
...
pub fn run_module_goal_test() {
  let assert Error(err) =
    arc_aot.execute(Run("test/fixtures/esm.js"), aot_harness.test_hooks())
  assert err
...
pub fn run_missing_file_test() {
  let assert Error(arc_aot.ReadFailed(path: "nope.js", ..) as err) =
    arc_aot.execute(Run("nope.js"), aot_harness.test_hooks())
  assert arc_aot.exit_code(err) == 1
}

pub fn usage_error_test() {
  let assert Error(err) =
    arc_aot.execute(arc_aot.parse_args(["build"]), aot_harness.test_hooks())
  assert arc_aot.exit_code(err) == 2
