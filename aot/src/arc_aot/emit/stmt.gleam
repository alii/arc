...
import arc/compiler/scope.{
  type Binding, type ScopeId, CaptureBinding, ConstBinding, FnNameBinding,
...
import arc_aot/emit/state.{
  type BarrierCleanup, type EmitError, type EmitResult, type Emitter, type Next,
  type NextWith,
}
...
fn find_frame_carried(e: Emitter, ir_label: String) -> List(Int) {
...
fn frame_carried_walk(
  ...
  case frames {
    ...
    [frame, ..rest] ->
      case frame {
        state.LoopFrame(ir_break:, ir_continue:, carried:, ..)
          if ir_break == ir_label || ir_continue == ir_label
        -> carried
        state.SwitchFrame(ir_break:, carried:, ..) if ir_break == ir_label ->
          carried
        state.LabeledBlockFrame(ir_break:, carried:, ..)
          if ir_break == ir_label
        -> carried
...
fn inline_cleanup(e: Emitter, cleanup: BarrierCleanup, k: Next) -> EmitResult {
  let abrupt = e.consts.false_
  case cleanup {
    state.FinallyBlock(body:, saved_scope:, escape: esc) -> {
      let carried = assigned_unboxed_slots_all(e, [ast.BlockStatement(body)])
      use #(f_tree, e) <- result.try(
        cps.with_done(e, fn(done, e) {
          use e <- try_finally.inline_finally(e, body, saved_scope)
    ...
    state.IterClose(iter_var:, escape: esc) -> {
      let close =
        ir.Let(
          ["_"],
          ir.CallHost("js", "iter_close", [ir.Var(iter_var), abrupt]),
          ir.Values([]),
        )
...
fn keep_frames(r: EmitResult, frames: List(state.Frame)) -> EmitResult {
  use #(tree, e) <- result.map(r)
  #(tree, state.Emitter(..e, frame_stack: frames))
}

fn machine_goto(e: Emitter, ir_label: String) -> Option(EmitResult) {
  case e.machine_abrupt {
    Some(hooks) -> hooks.on_goto(e, ir_label)
    None -> None
  }
}
...
fn emit_return(e: Emitter, arg: Option(ast.Expression)) -> EmitResult {
  ...
  case arg {
    ...
    Some(ex) -> {
      ...
      cps.let_(e, rhs, with_value)
    }
  }
}

fn expr_(e: Emitter, ex: ast.Expression, k: NextWith(ir.Value)) -> EmitResult {
...
fn fold_body(
  ...
  cps.each(e, body, then: next, with: fn(e, located, k) {
    ...
    _ ->
      case ast_util.block_has_declarations(body) {
        ...
        True -> {
          ...
          use e <- func.hoist_fn_decls(e, body)
...
fn annexb_promote(e: Emitter, name: String, k: Next) -> EmitResult {
...
fn emit_var_decl(
  ...
  use e, decl, next <- cps.each(e, decls, then: next)
  ...
  case callees {
    ...
    [callee, ..rest] -> {
      ...
      case callee {
        ...
        state.BoxedSlot(_) -> cps.host(e, "box_get", [sv], go)
      }
    }
  }
}
...
fn loop_invariant_callees(
  ...
  |> list.unique
  |> list.sort(fn(a, b) { int.compare(a.slot, b.slot) })
}
...
fn per_iteration_env(
  ...
  case ast_util.for_head_lex_names(left) {
    [] -> k(e)
    _ -> func.binding_prologue(e, head_scope, k)
  }
}
...
fn for_lhs_ident_assign(
  ...
  case state.resolve(e, name) {
    ...
    scope.Plain(scope.Local(slot:, boxed:, kind:, ..)) -> {
      ...
      let checked = fn(e: Emitter) {
        let read = fn(e, kk) {
          case boxed {
            True ->
              cps.host(e, "box_get", [ir.Var(state.get_slot_var(e, slot))], kk)
            False -> kk(ir.Var(state.get_slot_var(e, slot)), e)
          }
        }
        use cur, e <- read(e)
        use e <- cps.host_unit(e, "tdz_check", [
          cur,
      ...
      case kind {
        ...
        LetBinding ->
          case set.contains(e.initialized_slots, slot) {
            True -> write(e)
            False -> checked(e)
          }
        _ -> write(e)
  ...
  case property {
    ast.Dot(name: "#" <> _ as name, ..) ->
      case state.resolve(e, name) {
        ...
        scope.Plain(scope.Local(slot:, boxed: True, ..)) -> {
          use key, e <- cps.host(e, "box_get", [
            ir.Var(state.get_slot_var(e, slot)),
          ])
          cps.host_unit(e, "private_set", [base, key, v], k)
        }
        _ ->
    ...
    ast.Dot(name:, ..) -> {
      ...
      cps.host_unit(e, expr.set_prop_op_name(e.strict), [base, key, v], k)
    }
    ast.Bracket(expression:) -> {
      use kv, e <- expr_(e, expression)
      use key, e <- cps.host(e, "to_property_key", [kv])
      cps.host_unit(e, expr.set_prop_op_name(e.strict), [base, key, v], k)
    }
  }
}
...
fn emit_for_in(
  ...
  let seed_head = fn(e: Emitter, k) {
    case has_lex {
      True -> func.binding_prologue(e, e.cur_scope, k)
      False -> k(e)
    }
  }
  use e <- seed_head(e)
  ...
  use keys, e <- cps.host(e, "for_in_keys", [obj])
  ...
  use #(loop_body, e) <- result.try(
    cps.with_done(e, fn(done, e) {
      ...
      use empty, e <- cps.let_(e, ir.TermOp(ir.IsEmptyList, [ir.Var(tail_p)]))
      let brk_payload = carried_values(e, carried)
      use #(not_empty, e) <- result.try(
        cps.with_done(e, fn(done_ne, e) {
          use key, e <- cps.let_(e, ir.TermOp(ir.ListHead, [ir.Var(tail_p)]))
          use rest, e <- cps.let_(e, ir.TermOp(ir.ListTail, [ir.Var(tail_p)]))
          use e <- per_iteration_env(e, left, head_scope)
          use e <- for_lhs_bind(e, left, key)
          use #(cont_body, e) <- result.try(
            cps.with_done(e, fn(done_cb, e) {
  ...
  case tail {
    ...
    ast.TryFinally(finalizer) -> {
      ...
      use #(tree, e) <- result.try(
        try_finally.emit_try_finally(e, block, finalizer, fn(ef) {
          Ok(#(ir.Values([]), ef))
        }),
      )
      let e = state.Emitter(..e, slot_vars: slot_vars0)
    ...
    ast.TryCatchFinally(handler, finalizer) -> {
      let slot_vars0 = e.slot_vars
      use #(tree, e) <- result.try(
        try_finally.emit_try_catch_finally(e, block, handler, finalizer, fn(ef) {
          Ok(#(ir.Values([]), ef))
        }),
      )
      let e = state.Emitter(..e, slot_vars: slot_vars0)
...
fn emit_catch_handler(
  ...
  case param {
    Some(p) -> {
      ...
      use e <- try_finally.catch_binding_prologue(e, e.cur_scope)
      use #(dtree, e) <- result.try(e.dispatch.emit_destructure(
        e,
        p,
        ir.Var("_e"),
...
fn emit_switch(
  ...
  let #(save, e) = state.enter_scope(e, in_block: True)
  use e <- func.binding_prologue(e, e.cur_scope)
  use e <- func.hoist_fn_decls(e, all_stmts)
  let #(labelled_rev, e) =
    ...
    [CaseEntry(lbl:, body:, ..), ..rest] -> {
      ...
      use #(wrapped, e) <- result.try(
        rebind_after_block(
          ...
          fn(e) {
            use ef <- cps.each(e, body, with: fn(e, located, k) {
              emit_stmt(e, located.statement, k)
            })
            case rest {
...
fn resolve_per_iter(e: Emitter, names: List(String)) -> List(#(Int, Bool)) {
...
fn per_iter_rebox(
  ...
  use e, #(slot, boxed), next <- cps.each(e, per_iter, then: k)
  case boxed {
    False -> next(e)
    True -> {
      let old = ir.Var(state.get_slot_var(e, slot))
      use v, e <- cps.host(e, "box_get", [old])
      use box, e <- cps.host(e, "box_new", [v])
      let #(n, e) = state.fresh_slot_var(e, slot)
...
fn emit_for_classic(
  ...
  use e <- seed_head(e)
  let after_init = fn(e: Emitter, per_iter_names: List(String)) {
    ...
    use #(loop_body, e) <- result.try(
      cps.with_done(e, fn(done, e) {
        ...
        let then_part = fn(e) {
          cps.with_done(e, fn(d2, e) {
            use #(cont_body, e) <- result.try(
              cps.with_done(e, fn(d3, e) {
                use e <- emit_stmt(e, body)
    ...
    let e = state.pop_frame(e)
    let e = state.Emitter(..e, invariant_callees: prev_callees)
...
fn emit_cond_i32(
  ...
  let #(tree, e) = anf.run(expr.emit_cond_i32(cond), e)
  cps.let_(e, tree, k)
}
...
fn emit_while(
  ...
  use #(loop_body, e) <- result.try(
    cps.with_done(e, fn(done, e) {
      ...
      let brk_payload = carried_values(e, carried)
      use #(then_tree, e) <- result.try(
        cps.with_done(e, fn(done_t, e) {
          use #(cont_body, e) <- result.try(
            cps.with_done(e, fn(done2, e) {
              use e <- emit_stmt(e, body)
