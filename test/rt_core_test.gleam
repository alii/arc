...
fn this_fn(st: Agent, strict strict: Bool) -> #(JsVal, Agent) {
  ...
  let #(h, st) = rt_call.fn_new(st, code, flags(strict), "f", 0, None, None)
  #(mk_object(h), st)
}
...
pub fn sloppy_this_boxes_primitives_test() {
  ...
  let #(r, st) = rt_call.call(st, f, mk_int(5), [])
  assert type_of(st, r) == "object"
  let #(v, st) = rt_val.to_number(st, r)
  assert v == JInt(5)
  let #(r, st) = rt_call.call(st, f, mk_string("s"), [])
  assert type_of(st, r) == "object"
  let #(r, st) = rt_call.call(st, f, mk_undefined(), [])
  assert r == mk_object(st.realm.global_object)
  let #(r, st) = rt_call.call(st, f, mk_null(), [])
  assert r == mk_object(st.realm.global_object)
  let #(o, st) = rt_obj.new_object_literal(st)
  let #(r, _) = rt_call.call(st, f, o, [])
  assert r == o
}
...
pub fn strict_this_passes_through_test() {
  ...
  let #(r, _) = rt_call.call(st, f, mk_null(), [])
  assert r == mk_null()
}
...
fn show(st: Agent, v: JsVal) -> String {
  rt_val.to_string(st, v).0
}

fn is_minus_zero(st: Agent, v: JsVal) -> Bool {
  let #(q, _) = rt_ops.div(st, mk_int(1), v)
  num(q) == JNegInf
}
...
pub fn integer_results_widen_past_2_53_test() {
  ...
  let #(a, st) = rt_ops.add(st, m, mk_int(1))
  assert num(a) == JFloat(9_007_199_254_740_992.0)
  let #(b, st) = rt_ops.add(st, m, mk_int(2))
  assert rt_val.strict_eq(a, b)
  assert show(st, b) == "9007199254740992"
  let #(c, st) = rt_ops.add(st, b, mk_int(1))
  assert show(st, c) == "9007199254740992"
  let #(d, st) = rt_ops.sub(st, rt_ops.neg(st, m).0, mk_int(2))
  assert show(st, d) == "-9007199254740992"
  let #(e, st) = rt_ops.mul(st, m, m)
  assert show(st, e) == "8.112963841460666e+31"
  let #(f, st) = rt_ops.mul(st, mk_int(123_456_789), mk_int(987_654_321))
...
pub fn float_overflow_is_infinity_test() {
  ...
  let #(a, st) = rt_ops.mul(st, big, mk_int(10))
  assert num(a) == JPosInf
  let #(b, st) = rt_ops.mul(st, rt_ops.neg(st, big).0, mk_int(10))
  assert num(b) == JNegInf
  let #(c, st) = rt_ops.add(st, big, big)
  assert num(c) == JPosInf
  let #(d, st) = rt_ops.sub(st, rt_ops.neg(st, big).0, big)
  assert num(d) == JNegInf
  let #(e, st) = rt_ops.mul(st, max, mk_int(2))
  assert num(e) == JPosInf
  let #(f, st) = rt_ops.div(st, big, mk_number(JFloat(1.0e-10)))
  assert num(f) == JPosInf
  let #(g, st) = rt_ops.div(st, big, mk_number(JFloat(-1.0e-10)))
  assert num(g) == JNegInf
  let #(h, st) = rt_ops.pow(st, mk_int(-10), mk_int(401))
...
pub fn minus_zero_survives_integer_arithmetic_test() {
  let st = agent()
  let #(a, st) = rt_ops.mul(st, mk_int(0), mk_int(-1))
  assert is_minus_zero(st, a)
  let #(b, st) = rt_ops.mul(st, mk_int(-7), mk_int(0))
  assert is_minus_zero(st, b)
  let #(c, st) = rt_ops.mul(st, mk_int(0), mk_int(3))
  assert !is_minus_zero(st, c)
  let #(d, st) = rt_ops.neg(st, mk_int(0))
  assert is_minus_zero(st, d)
  let #(e, st) = rt_ops.add(st, d, d)
  assert is_minus_zero(st, e)
  let #(f, st) = rt_ops.add(st, d, mk_int(0))
  assert !is_minus_zero(st, f)
  let #(g, st) = rt_ops.sub(st, mk_int(0), mk_int(0))
  assert !is_minus_zero(st, g)
  let #(h, st) = rt_ops.mod(st, mk_int(-4), mk_int(2))
  assert is_minus_zero(st, h)
  let #(i, st) = rt_ops.mod(st, mk_int(4), mk_int(-2))
  assert !is_minus_zero(st, i)
  let #(j, st) = rt_ops.div(st, mk_int(0), mk_int(-5))
...
fn global(st: Agent, name: String) -> JsVal {
  rt_lang.global_get(st, <<name:utf8>>).0
}

fn get(st: Agent, obj: JsVal, name: String) -> JsVal {
  rt_obj.get_prop(st, obj, StringKey(key.canonical(name))).0
}

fn error_stack(st: Agent, msg: String) -> String {
  let ctor = global(st, "Error")
  let #(h, st) = rt_call.construct(st, ctor, [mk_string(msg)], ctor)
...
pub fn error_stack_renders_frames_test() {
  ...
  let #(h, st2) = rt_call.construct(st, type_error, [], type_error)
...
pub fn bytecode_call_and_construct_use_js_ops_test() {
  ...
  let ops =
    JsOps(
      ...
      call_bytecode: fn(st, _callee, _kind, this, args) {
        let assert [a] = args
        let #(sum, st) = rt_ops.add(st, this, a)
        #(Ok(sum), st)
      },
  ...
  let #(r, st) = rt_call.call(st, f, mk_int(40), [mk_int(2)])
  assert classify(r) == KNum(JInt(42))
  let #(h, st) = rt_call.construct(st, f, [], f)
...
pub fn weak_map_value_traced_until_key_dies_test() {
  ...
  let #(wm_h, st) = rt_call.construct(st, wm_ctor, [], wm_ctor)
  let wm = mk_object(wm_h)
  let #(k, st) = rt_obj.new_object_literal(st)
  let #(v, st) = rt_obj.new_object_literal(st)
  let #(_, st) =
    rt_call.call_method(st, wm, StringKey(key.canonical("set")), [k, v])
  let st = rt_gc.collect(st, [wm_h, handle(k)])
  assert rt_gc.is_live(st, handle(v))
  let #(got, st) =
    rt_call.call_method(st, wm, StringKey(key.canonical("get")), [k])
  assert got == v
  let st = rt_gc.collect(st, [wm_h])
  assert !rt_gc.is_live(st, handle(k))
  let #(has, st) =
    rt_call.call_method(st, wm, StringKey(key.canonical("has")), [k])
...
fn call_method(st: Agent, recv: JsVal, name: String, args: List(JsVal)) {
  rt_call.call_method(st, recv, StringKey(key.canonical(name)), args)
}

pub fn map_get_or_insert_test() {
  let st = agent()
  let map_ctor = global(st, "Map")
  let #(mh, st) = rt_call.construct(st, map_ctor, [], map_ctor)
  let m = mk_object(mh)
  ...
  let #(size, st) = rt_obj.get_prop(st, m, StringKey(key.canonical("size")))
  assert classify(size) == KNum(JInt(2))
  let assert #(ThrowCompletion(_), _) =
    rt_call.try_call(st, get(st, m, "getOrInsert"), mk_string("not a map"), [])
}
...
pub fn map_get_or_insert_computed_test() {
  ...
  let seen_key =
    as_code(fn(st, _frame, args) {
      let assert [k] = args
      let #(q, st) = rt_ops.div(st, mk_int(1), k)
      #(mk_string(rt_val.to_string(st, q).0), st)
    })
  let #(fh, st) =
    rt_call.fn_new(st, seen_key, flags(strict: True), "f", 1, None, None)
  let f = mk_object(fh)
  let #(mz, st) = rt_ops.neg(st, mk_int(0))
  ...
  let #(bh, st) =
    rt_call.fn_new(st, boom, flags(strict: True), "b", 1, None, None)
  let #(r, st) =
  ...
  let #(sh, st) =
    rt_call.fn_new(st, sneaky, flags(strict: True), "s", 1, None, None)
  let #(r, st) =
  ...
  assert classify(size) == KNum(JInt(2))
  let assert #(ThrowCompletion(_), _) =
    rt_call.try_call(st, get(st, m, "getOrInsertComputed"), m, [
      mk_int(0),
      mk_int(1),
    ])
}
...
pub fn map_group_by_test() {
  ...
  let parity =
    as_code(fn(st, _frame, args) {
      let assert [n, ..] = args
      let #(r, st) = rt_ops.mod(st, n, mk_int(2))
      case classify(r) {
        KNum(JInt(0)) -> #(mk_string("even"), st)
        KNum(JInt(_)) -> #(mk_string("odd"), st)
        _ -> rt_ops.neg(st, mk_int(0))
      }
    })
  let #(ph, st) =
    rt_call.fn_new(st, parity, flags(strict: True), "p", 1, None, None)
  ...
  let assert #(ThrowCompletion(_), _) =
    rt_call.try_call(st, get(st, map_ctor, "groupBy"), map_ctor, [
      items,
      mk_int(1),
    ])
}
