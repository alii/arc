...
import arc/rt/types.{
  ...
  StringKey, canonical_key, classify, mk_bigint, mk_bool, mk_int, mk_number,
...
pub fn float64_over_buffer_with_byte_offset_test() {
  let st = agent()
  let #(buf, st) = construct(st, "ArrayBuffer", [mk_int(24)])
  let #(f, st) = construct(st, "Float64Array", [buf, mk_int(8)])
...
pub fn out_of_bounds_read_undefined_write_ignored_test() {
  let st = agent()
  let #(ta, st) = construct(st, "Int16Array", [mk_int(2)])
  assert classify(get_(st, ta, "5")) == KUndef
  assert classify(get_(st, ta, "-1")) == KUndef
  let st = set(st, ta, "5", mk_int(9))
  let st = set(st, ta, "1.5", mk_int(9))
  assert classify(get_(st, ta, "5")) == KUndef
...
pub fn resizable_buffer_length_tracking_view_test() {
  let st = agent()
  let #(opts, st) = options(st, "maxByteLength", mk_int(16))
  let #(buf, st) = construct(st, "ArrayBuffer", [mk_int(4), opts])
  ...
  let #(_, st) = invoke(st, buf, "resize", [mk_int(12)])
  assert classify(get_(st, view, "length")) == KNum(JInt(12))
  let st = set(st, view, "11", mk_int(5))
  assert classify(get_(st, view, "11")) == KNum(JInt(5))
  let #(_, st) = invoke(st, buf, "resize", [mk_int(2)])
  assert classify(get_(st, view, "length")) == KNum(JInt(2))
  assert classify(get_(st, view, "11")) == KUndef
  let #(_, st) = invoke(st, buf, "resize", [mk_int(8)])
  let #(fixed, st) = construct(st, "Uint8Array", [buf, mk_int(4), mk_int(4)])
  let #(_, st) = invoke(st, buf, "resize", [mk_int(6)])
...
pub fn transfer_detaches_source_test() {
  let st = agent()
  let #(buf, st) = construct(st, "ArrayBuffer", [mk_int(8)])
  let #(view, st) = construct(st, "Uint8Array", [buf])
  let st = set(st, view, "0", mk_int(3))
  ...
  assert throws(st, view, "fill", [mk_int(1)]) == "TypeError"
}
...
pub fn data_view_int16_endianness_and_float16_test() {
  ...
  let #(_, st) =
    invoke(st, dv, "setInt16", [mk_int(0), mk_int(-2), mk_bool(True)])
  let #(le, st) = invoke(st, dv, "getInt16", [mk_int(0), mk_bool(True)])
  // fe ff big-endian signed is -257
  let #(be, st) = invoke(st, dv, "getInt16", [mk_int(0), mk_bool(False)])
  assert classify(le) == KNum(JInt(-2))
  assert classify(be) == KNum(JInt(-257))
  let #(_, st) =
    invoke(st, dv, "setFloat16", [mk_int(2), mk_number(JFloat(1.5))])
  let #(h, st) = invoke(st, dv, "getFloat16", [mk_int(2)])
  assert classify(h) == KNum(JFloat(1.5))
  let #(_, st) = invoke(st, dv, "setFloat16", [mk_int(4), mk_int(65_520)])
  let #(inf, st) = invoke(st, dv, "getFloat16", [mk_int(4)])
  assert classify(inf) == KNum(types.JPosInf)
  assert throws(st, dv, "getFloat64", [mk_int(4)]) == "RangeError"
}

pub fn bigint64_array_test() {
  let st = agent()
  let #(ta, st) = construct(st, "BigInt64Array", [mk_int(2)])
  ...
  let #(c, st) =
    rt_call.try_run(st, fn(st) {
      #(mk_undefined(), set(st, ta, "0", mk_int(1)))
    })
  let assert ThrowCompletion(err) = c
...
pub fn atomics_on_shared_int32_test() {
  let st = agent()
  let #(sab, st) = construct(st, "SharedArrayBuffer", [mk_int(8)])
  let #(ta, st) = construct(st, "Int32Array", [sab])
  let #(old, st) = static(st, "Atomics", "add", [ta, mk_int(0), mk_int(5)])
  assert classify(old) == KNum(JInt(0))
  let #(v, st) = static(st, "Atomics", "load", [ta, mk_int(0)])
  assert classify(v) == KNum(JInt(5))
  let #(w, st) =
    static(st, "Atomics", "compareExchange", [
      ta,
      mk_int(0),
      mk_int(5),
      mk_int(9),
  ...
  let #(woken, st) = static(st, "Atomics", "notify", [ta, mk_int(0), mk_int(1)])
  assert classify(woken) == KNum(JInt(0))
  let atomics = global(st, "Atomics")
  assert throws(st, atomics, "wait", [ta, mk_int(0), mk_int(0), mk_int(0)])
    == "TypeError"
  assert throws(st, atomics, "load", [ta, mk_int(2)]) == "RangeError"
}
...
pub fn set_subarray_slice_sort_at_test() {
  ...
  let #(_, st) = invoke(st, x, "set", [nine, mk_int(2)])
  assert joined(st, x) == "5,1,9"
  let #(sub, st) = invoke(st, x, "subarray", [mk_int(1)])
  assert joined(st, sub) == "1,9"
  assert get_(st, sub, "buffer") == get_(st, x, "buffer")
  let #(sl, st) = invoke(st, x, "slice", [mk_int(0), mk_int(2)])
  ...
  let #(last, st) = invoke(st, x, "at", [mk_int(-1)])
  assert classify(last) == KNum(JFloat(9.0))
  let #(none, _) = invoke(st, x, "at", [mk_int(3)])
  assert classify(none) == KUndef
}

pub fn wide_integer_float_store_rounds_like_arithmetic_test() {
  let st = agent()
  let #(f, st) = construct(st, "Float64Array", [mk_int(2)])
  let wide = 18_446_744_073_709_553_665
  let st = set(st, f, "0", mk_int(wide))
  assert classify(get_(st, f, "0")) == KNum(rt_val.num_from_int(wide))
  let st = set(st, f, "1", mk_int(9_007_199_254_740_993))
