...
pub fn iterator_record_steps_then_stays_done_test() {
  ...
  let #(rec, st) = rt_lang.t_get_iterator(st, arr, rt_lang.Sync)
  let #(#(d1, v1), st) = rt_lang.t_iter_next(st, rec)
  let #(#(d2, v2), st) = rt_lang.t_iter_next(st, rec)
  let #(#(d3, v3), st) = rt_lang.t_iter_next(st, rec)
  let #(#(d4, _), st) = rt_lang.t_iter_next(st, rec)
  assert !d1 && !d2 && d3 && d4
  assert classify(v1) == KNum(JInt(7))
  assert classify(v2) == KNum(JInt(8))
  assert classify(v3) == KUndef
  let st = rt_lang.t_iter_close(st, rec, abrupt: False)
  let _st = rt_lang.t_iter_close(st, rec, abrupt: True)
}
...
pub fn iter_rest_and_spread_test() {
  ...
  let #(rec, st) = rt_lang.t_get_iterator(st, arr, rt_lang.Sync)
  let #(_, st) = rt_lang.t_iter_next(st, rec)
  let #(rest, st) = rt_lang.t_iter_rest(st, rec)
  let #(len, st) = rt_obj.t_get_prop(st, rest, StringKey(Named("length")))
  assert classify(len) == KNum(JInt(2))
  let #(spread, _st) = rt_lang.t_spread_into_list(st, [mk_string("a")], arr)
...
pub fn object_rest_excludes_keys_test() {
  ...
  let #(rest, st) = rt_lang.t_object_rest(st, src, [StringKey(Named("a"))])
...
pub fn template_object_is_cached_per_site_test() {
  ...
  let #(t1, st) =
    rt_lang.t_get_template_object(st, "m#11", cooked, ["a", "\\u"])
  let #(t2, st) =
    rt_lang.t_get_template_object(st, "m#11", cooked, ["a", "\\u"])
  let #(t3, st) =
    rt_lang.t_get_template_object(st, "m#12", cooked, ["a", "\\u"])
  assert t1 == t2
  assert t1 != t3
