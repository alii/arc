...
fn get(st: Agent, obj: JsVal, name: String) -> #(JsVal, Agent) {
  rt_obj.get_prop(st, obj, key(name))
}
...
fn set(st: Agent, obj: JsVal, name: String, v: JsVal) -> Agent {
  let #(_, st) = rt_obj.set_prop(st, obj, key(name), v)
  st
}

fn object(st: Agent) -> #(JsVal, Agent) {
  rt_obj.new_object_literal(st)
}
...
fn func(
  ...
  let #(h, st) = rt_call.new_builtin_function(st, "", 0, body)
  #(mk_object(h), st)
}
...
fn static(
  ...
  rt_call.call(st, f, global(st, ns), args)
}

fn proxy(st: Agent, target: JsVal, handler: JsVal) -> #(JsVal, Agent) {
  let ctor = global(st, "Proxy")
  let #(h, st) = rt_call.construct(st, ctor, [target, handler], ctor)
  #(mk_object(h), st)
}
  ...
  let st =
    list.fold(names, st, fn(st, name) {
      let #(trap, st) =
        func(st, fn(st, args) {
          let #(_, st) =
            rt_call.call_method(st, log, key("push"), [mk_string(name)])
...
fn drain(st: Agent, log: JsVal) -> #(String, Agent) {
  let #(joined, st) =
    rt_call.call_method(st, log, key("join"), [mk_string(",")])
...
pub fn each_internal_method_reaches_its_trap_test() {
  let st = agent()
  let #(log, st) = rt_obj.new_array(st, [])
  ...
  let #(has, st) = rt_obj.has_prop(st, p, key("a"))
  assert has
  let #(seen, st) = drain(st, log)
  assert seen == "has"

  let #(ok, st) = rt_obj.delete_prop(st, ph, key("b"))
  assert ok
  let #(seen, st) = drain(st, log)
  assert seen == "deleteProperty"

  let #(_, st) = rt_obj.own_keys(st, ph)
  let #(seen, st) = drain(st, log)
  assert seen == "ownKeys"

  let #(_, st) = rt_obj.get_own_property(st, ph, key("a"))
  let #(seen, st) = drain(st, log)
  ...
  let #(_, st) = rt_obj.get_prototype_of(st, ph)
  let #(seen, st) = drain(st, log)
  assert seen == "getPrototypeOf"

  let #(res, st) = rt_obj.set_prototype_of(st, ph, None)
  assert res == Ok(Nil)
  let #(seen, st) = drain(st, log)
  assert seen == "setPrototypeOf"
  let #(proto, st) = rt_obj.get_prototype_of(st, handle(target))
  assert proto == None
  let #(_, st) = drain(st, log)

  let #(ext, st) = rt_obj.is_extensible(st, ph)
  assert ext
  let #(seen, st) = drain(st, log)
  assert seen == "isExtensible"

  let #(ok, st) = rt_obj.prevent_extensions(st, ph)
  assert ok
  let #(seen, st) = drain(st, log)
  assert seen == "preventExtensions"
  let #(ext, _) = rt_obj.is_extensible(st, handle(target))
  assert !ext
}
...
pub fn call_and_construct_traps_test() {
  ...
  let #(p, st) = proxy(st, target, handler)
  let #(v, st) = rt_call.call(st, p, mk_undefined(), [mk_int(41)])
  assert classify(v) == KNum(JInt(42))
  let #(seen, st) = drain(st, log)
  assert seen == "apply"
  let #(pa, st) = proxy(st, global(st, "Array"), handler)
  let #(arr_h, st) = rt_call.construct(st, pa, [mk_int(3)], pa)
  let #(seen, st) = drain(st, log)
  ...
  assert throws(st, fn(st) { rt_call.construct(st, pb, [], pb) }) == "TypeError"
  let #(plain, st) = object(st)
  let #(pp, st) = proxy(st, plain, handler)
  assert throws(st, fn(st) {
      rt_call.try_call(st, pp, mk_undefined(), []) |> ok
    })
    == "TypeError"
}
...
fn ok(r: #(rt_call.Completion(JsVal), Agent)) -> #(JsVal, Agent) {
  case r {
    ...
    #(ThrowCompletion(e), st) -> rt_call.call(st, e, e, [])
  }
}
...
pub fn absent_traps_forward_to_target_test() {
  ...
  let st = set(st, p, "b", mk_int(2))
  assert classify(get_(st, target, "b")) == KNum(JInt(2))
  let #(has, st) = rt_obj.has_prop(st, p, key("b"))
  assert has
  let #(keys, st) = rt_obj.own_keys(st, ph)
  assert keys == [key("a"), key("b")]
  let #(ok, st) = rt_obj.delete_prop(st, ph, key("a"))
  assert ok
  let #(has, st) = rt_obj.has_prop(st, target, key("a"))
  assert !has
  let #(proto, st) = rt_obj.get_prototype_of(st, ph)
...
pub fn object_keys_uses_own_keys_then_descriptors_test() {
  ...
  let #(joined, st) =
    rt_call.call_method(st, keys, key("join"), [mk_string(",")])
...
pub fn json_stringify_through_proxy_test() {
  ...
  let #(arr, st) = rt_obj.new_array(st, [mk_int(1), mk_int(2)])
...
pub fn is_array_sees_through_proxies_test() {
  let st = agent()
  let #(arr, st) = rt_obj.new_array(st, [])
  let #(empty, st) = object(st)
  ...
  let #(_, st) = rt_call.call_method(st, rv, key("revoke"), [])
...
pub fn instanceof_uses_get_prototype_of_trap_test() {
  ...
  let #(target, st) = object(st)
  let #(p, st) = proxy(st, target, handler)
  let #(r, st) = rt_ops.instance_of(st, p, global(st, "Object"))
  assert r
  let #(seen, st) = drain(st, log)
  assert seen == "getPrototypeOf"
  ...
  let #(r, st) = rt_ops.instance_of(st, p2, global(st, "Array"))
  assert r
  let #(is_proto, _) =
    rt_call.call_method(st, array_proto, key("isPrototypeOf"), [p2])
...
pub fn revoked_proxy_throws_on_every_operation_test() {
  ...
  let #(_, st) = rt_call.call_method(st, rv, key("revoke"), [])
  assert throws(st, get(_, p, "x")) == "TypeError"
  assert throws(st, rt_obj.set_prop(_, p, key("x"), mk_int(1))) == "TypeError"
  assert throws(st, rt_obj.has_prop(_, p, key("x"))) == "TypeError"
  assert throws(st, rt_obj.delete_prop(_, ph, key("x"))) == "TypeError"
  assert throws(st, rt_obj.own_keys(_, ph)) == "TypeError"
  assert throws(st, rt_obj.get_own_property(_, ph, key("x"))) == "TypeError"
  assert throws(st, fn(st) {
      rt_obj.define_own_data(
        st,
        ph,
    ...
    == "TypeError"
  assert throws(st, rt_obj.get_prototype_of(_, ph)) == "TypeError"
  assert throws(st, rt_obj.set_prototype_of(_, ph, None)) == "TypeError"
  assert throws(st, rt_obj.is_extensible(_, ph)) == "TypeError"
  assert throws(st, rt_obj.prevent_extensions(_, ph)) == "TypeError"
  assert throws(st, fn(st) { rt_call.try_call(st, p, mk_undefined(), []) |> ok })
    == "TypeError"
  assert throws(st, rt_call.construct(_, p, [], p)) == "TypeError"
  assert throws(st, rt_obj.for_in_keys(_, p)) == "TypeError"
...
fn locked_target(st: Agent, sealed sealed: Bool) -> #(JsVal, Agent) {
  ...
  let st = case sealed {
    True -> rt_obj.prevent_extensions(st, handle(t)).1
    False -> st
  }
  #(t, st)
}
...
pub fn set_invariant_test() {
  ...
  let #(p, st) = proxy(st, t, h)
  assert throws(st, rt_obj.set_prop(_, p, key("k"), mk_int(2))) == "TypeError"
  let #(ok, st) = rt_obj.set_prop(st, p, key("k"), mk_int(1))
  assert ok
  let #(h, st) = handler_of(st, "set", mk_bool(False))
  let #(p, st) = proxy(st, t, h)
  let #(ok, _) = rt_obj.set_prop(st, p, key("other"), mk_int(2))
  assert !ok
}
...
pub fn has_invariant_test() {
  ...
  let #(p, st) = proxy(st, t, h)
  assert throws(st, rt_obj.has_prop(_, p, key("k"))) == "TypeError"
  let #(t2, st) = record(st, [#("j", mk_int(1))])
  let st = rt_obj.prevent_extensions(st, handle(t2)).1
  let #(p2, st) = proxy(st, t2, h)
  assert throws(st, rt_obj.has_prop(_, p2, key("j"))) == "TypeError"
  let #(has, _) = rt_obj.has_prop(st, p2, key("zz"))
  assert !has
}
...
pub fn delete_invariant_test() {
  ...
  let #(p, st) = proxy(st, t, h)
  assert throws(st, rt_obj.delete_prop(_, handle(p), key("k"))) == "TypeError"
  let #(t2, st) = record(st, [#("j", mk_int(1))])
  let st = rt_obj.prevent_extensions(st, handle(t2)).1
  let #(p2, st) = proxy(st, t2, h)
  assert throws(st, rt_obj.delete_prop(_, handle(p2), key("j"))) == "TypeError"
}
...
pub fn get_own_property_descriptor_invariants_test() {
  ...
  let #(p, st) = proxy(st, t, h)
  assert throws(st, rt_obj.get_own_property(_, handle(p), key("k")))
    == "TypeError"
  ...
  let #(p, st) = proxy(st, t, h)
  assert throws(st, rt_obj.get_own_property(_, handle(p), key("nope")))
    == "TypeError"
  ...
  let #(p, st) = proxy(st, t, h)
  let #(desc, _) = rt_obj.get_own_property(st, handle(p), key("k"))
...
pub fn own_keys_invariants_test() {
  let st = agent()
  let #(t, st) = locked_target(st, sealed: False)
  let #(empty_arr, st) = rt_obj.new_array(st, [])
  let #(h, st) = handler_of(st, "ownKeys", empty_arr)
  let #(p, st) = proxy(st, t, h)
  assert throws(st, rt_obj.own_keys(_, handle(p))) == "TypeError"
  let #(dups, st) = rt_obj.new_array(st, [mk_string("k"), mk_string("k")])
  let #(h, st) = handler_of(st, "ownKeys", dups)
  let #(p, st) = proxy(st, t, h)
  assert throws(st, rt_obj.own_keys(_, handle(p))) == "TypeError"
  let #(bad, st) = rt_obj.new_array(st, [mk_string("k"), mk_int(1)])
  let #(h, st) = handler_of(st, "ownKeys", bad)
  let #(p, st) = proxy(st, t, h)
  assert throws(st, rt_obj.own_keys(_, handle(p))) == "TypeError"
  let #(t2, st) = locked_target(st, sealed: True)
  let #(extra, st) = rt_obj.new_array(st, [mk_string("k"), mk_string("zz")])
  ...
  let #(p, st) = proxy(st, t2, h)
  let #(keys, _) = rt_obj.own_keys(st, handle(p))
  assert keys == [key("k")]
}
...
pub fn proxy_as_prototype_traps_on_inherited_access_test() {
  ...
  let #(handler, st) = logging_handler(st, log)
  let #(p, st) = proxy(st, target, handler)
  let #(child_h, st) = rt_obj.new_object(st, Some(handle(p)))
  let child = mk_object(child_h)
  let #(v, st) = get(st, child, "inherited")
  assert classify(v) == KNum(JInt(9))
  let #(seen, st) = drain(st, log)
  assert seen == "get"
  let #(has, st) = rt_obj.has_prop(st, child, key("inherited"))
  assert has
  let #(seen, st) = drain(st, log)
  assert seen == "has"
  ...
  let #(seen, st) = drain(st, log)
  assert seen == "set"
  let #(own, _) = rt_obj.get_own_property(st, child_h, key("fresh"))
