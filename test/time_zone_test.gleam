...
fn primary(id: String) -> String {
  let assert Some(identifier) = time_zone.known_identifier(id)
  time_zone.primary_identifier(identifier)
}
...
pub fn lookup_is_ascii_case_insensitive_test() {
  ...
  assert time_zone.known_identifier("Asia/Kol\u{212A}ata") == None
}

pub fn unknown_id_rejected_test() {
  assert time_zone.known_identifier("Asia/Nowhere") == None
  assert time_zone.known_identifier("IST") == None
  assert time_zone.known_identifier("Factory") == None
  assert time_zone.known_identifier("posixrules") == None
  assert time_zone.known_identifier("") == None
}
...
pub fn available_ids_are_primary_and_sorted_test() {
  let ids = time_zone.available_ids(zoneinfo.available_ids())
