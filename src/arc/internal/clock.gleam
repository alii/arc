...
////
//// Nothing in the runtime should call these directly on a hot path: the
//// engine reads the clock through `host_hooks.HostHooks.monotonic_now` /
...
/// idle: an untimed sync `Atomics.wait` blocks in the SharedArrayBuffer's
/// owner protocol (`arc/rt/sab`), never here.
