...
////
//// Read-only heap walk that renders a `JsValue` for humans (REPL echo,
//// `console.log`, uncaught-exception reports). Never re-enters the VM: does
//// NOT call `toString`/`valueOf`/`@@toPrimitive` or any user JS, so it cannot
//// throw and cannot mutate state. Lives outside `ops/object` because it is a
//// presentation concern, not part of the meta-object protocol.
...
import arc/vm/value.{
  type JsElements, type JsValue, ArrayObject, DataProperty, FunctionObject,
...
fn inspect_inner(
  ...
  case val {
    ...
    value.JsSymbol(id) -> value.symbol_descriptive_string(id)
    ...
    Some(ObjectSlot(kind:, properties:, elements:, symbol_properties:, ..)) ->
      case kind {
        ...
        value.TypedArrayObject(buffer:, elem_kind:, byte_offset:, length:) ->
          ...
          <> "("
          <> int.to_string(object.typed_array_view_length(
            heap,
            buffer,
            elem_kind,
            byte_offset,
