//// The old interpreter's realm-wide module registry, kept only while
//// `arc/vm` still runs; the public one is `arc/module/registry`. The
//// heap-resident caches the module system keeps on PRIVATE-keyed
//// global-object properties, invisible to guest JS.
////
...
//// (`arc/vm/module`), a deferred namespace's proxy traps (which fire while
//// some other module's body is mid-execution), and the dynamic-import host
//// hook (`arc/vm/module_host`). So that state lives in the heap: one hidden object
...
fn clear_entry(
  ...
  case object.get_own_property(h, global_object, property) {
    Some(DataProperty(value: JsObject(cache_ref), ..)) -> {
      ...
      case deleted {
        True -> h
        False -> panic as "arc/vm/module_registry: cache entry refused deletion"
      }
    }
