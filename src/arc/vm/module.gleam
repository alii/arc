//// The old interpreter's ES module system, kept only while `arc/vm` still
//// runs. The public module system is `arc/module`.
////
...
pub fn link_invariant_message(broken: LinkInvariantBroken) -> String {
  "arc/vm/module: linker invariant broken: " <> string.inspect(broken)
}
...
fn compile_source_module(
  ...
  let template = legacy.legacy_template(template)
  let export_seeds =
    dict.map_values(export_seeds, fn(_name, seed) { legacy.seed_value(seed) })
...
// accessors of `arc/vm/module_registry` — the single owner of every hidden
...
type EvalState(host) {
  EvalState(
    ...
    /// The realm-wide heap registry (`arc/vm/module_registry`) is the fallback
