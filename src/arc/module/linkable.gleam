...
pub fn resolve_export(
  ...
  resolve_export_visiting(graph, specifier, name, set.new())
}

fn resolve_export_visiting(
  graph: LinkableGraph,
  specifier: Resolved,
  name: String,
    ...
    list.find_map(m.export_entries, fn(e) {
      case e {
        ...
        ReExport(export_name:, imported_name:, dep:, ..)
          if export_name == name
        -> Ok(resolve_export_visiting(graph, dep, imported_name, resolve_set))
      ...
      list.find_map(bindings, fn(binding) {
        case binding {
          summary.NamedImport(local:, ..) if local == local_name ->
            Ok(#(dep, binding))
          summary.DefaultImport(local:) if local == local_name ->
            Ok(#(dep, binding))
          summary.NamespaceImport(local:, ..) if local == local_name ->
            Ok(#(dep, binding))
          _ -> Error(Nil)
        }
      })
    ...
    Ok(#(dep, binding)) ->
      case binding {
        summary.NamedImport(imported:, ..) ->
          resolve_export_visiting(graph, dep, imported, resolve_set)
        summary.DefaultImport(..) ->
          resolve_export_visiting(graph, dep, "default", resolve_set)
...
fn resolve_star_exports(
  ...
  list.fold(m.star_exports, Unresolvable, fn(acc, src) {
    case acc {
      Ambiguous -> Ambiguous
      _ ->
        case resolve_export_visiting(graph, src, name, resolve_set), acc {
...
pub fn exported_names(
  ...
  exported_names_visiting(graph, specifier, set.new())
}

fn exported_names_visiting(
  graph: LinkableGraph,
  spec: Resolved,
    ...
    False, Ok(m) -> {
      ...
      let star =
        list.flat_map(m.star_exports, fn(src) {
          exported_names_visiting(graph, src, star_set)
...
pub fn error_message(e: LinkError) -> String {
  case e {
    UnresolvedExport(requested_module:, export_name:) ->
      ...
      <> specifier.raw_text(requested_module)
  ...
  list.try_each(m.import_bindings, fn(entry) {
    ...
    list.try_each(bindings, fn(binding) {
      case binding {
        ...
        summary.DefaultImport(..) -> check_dep(graph, dep, raw_dep, "default")
      }
    })
  })
}
...
fn check_indirect_exports(
  ...
  list.try_each(m.export_entries, fn(e) {
    case e {
      ReExport(export_name: _, imported_name:, raw_dep:, dep:) ->
        check_dep(graph, dep, raw_dep, imported_name)
