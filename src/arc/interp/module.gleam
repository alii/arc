//// ES Module system for Arc on the shared runtime.
////
...
pub type CompiledModule {
  CompiledModule(
    ...
    /// Exported local name → whether its cell starts uninitialized (TDZ:
    /// let/const/class/default) rather than `undefined` (var/function).
...
fn compile_source_module(
  ...
  CompiledModule(
    specifier:,
    template: compiler.shared_template(body.template),
    ...
    requested_modules:,
    export_seeds: dict.map_values(body.export_seeds, fn(_name, seed) {
      export_seed(seed)
    }),
...
/// The compiler reports each seed as its own sentinel value; only two occur.
fn export_seed(seed: value.JsValue) -> ExportSeed {
  case seed {
    value.JsUninitialized -> SeedUninitialized
    _ -> SeedUndefined
  }
}
...
fn seed_value(seed: ExportSeed) -> JsVal {
  case seed {
    SeedUndefined -> mk_undefined()
    SeedUninitialized -> mk_tdz()
  }
}
