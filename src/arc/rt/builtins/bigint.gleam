...
fn bigint_global(st: Agent, args: List(JsVal)) -> #(JsVal, Agent) {
  ...
  let #(prim, st) = rt_val.to_primitive(st, arg, HintNumber)
  case classify(prim) {
    ...
    _ -> {
      let #(n, st) = rt_val.to_bigint(st, prim)
      #(mk_bigint(n), st)
    }
  }
}
...
fn bigint_as_int_n(st: Agent, args: List(JsVal)) -> #(JsVal, Agent) {
  ...
  let #(n, st) = rt_val.to_bigint(st, bigint_v)
  case bits {
    0 -> #(mk_bigint(0), st)
    _ -> {
...
fn bigint_proto_to_string(
  ...
  let #(radix, st) = case classify(radix_arg) {
    KUndef -> #(10, st)
    _ -> {
      let #(num, st) = rt_val.to_number(st, radix_arg)
