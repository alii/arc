...
// ============================================================================
// Wall-clock ↔ exact time in a zone (GetPossibleEpochNanoseconds,
// DisambiguatePossibleEpochNanoseconds, GetEpochNanosecondsFor, GetStartOfDay,
// InterpretISODateTimeOffset)
// ============================================================================

/// CheckISODaysRange — the wall-clock date must be within ±10^8 days.
pub fn check_iso_days_range(d: IsoDate) -> Result(Nil, TErr) {
  case int.absolute_value(temporal_iso.epoch_days(d)) <= 100_000_000 {
    True -> Ok(Nil)
...
pub fn get_possible_epoch_ns(
  ...
  case tz {
    ...
    TzOffset(ns: off) -> {
      ...
      use Nil <- result.try(
        check_iso_days_range(temporal_iso.iso_date_from_epoch_days(shifted_day)),
      )
...
pub fn start_of_day_ns(tz: TimeZone, d: IsoDate) -> Result(Int, TErr) {
  let midnight = temporal_iso.midnight
    ...
    OptionOffset(offset_ns) if offset_opt == UseOffset -> {
      ...
      use Nil <- result.try(
        check_iso_days_range(
          temporal_iso.iso_date_from_epoch_days(floor_div(ns, ns_per_day)),
        ),
      )
